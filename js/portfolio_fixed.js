...
function getCurrentLang() {
    return window.getCurrentLang ? window.getCurrentLang() : getSiteLang();
}

function isItemFavorited(itemId) {
    try {
        return localStorage.getItem('favorite-' + itemId) === 'true';
    } catch (e) {
        return false;
    }
}
...
function renderPortfolioGallery(items, container) {
    ...
    var html = items.map(function (p, index) {
        ...
        var catText = cat[lang] || cat.ar || '';
        var titleText = title[lang] || title.ar || '';
        var descText = (p.description && p.description[lang]) || p.description.ar || '';
        var shortDesc = galleryShortDesc(descText);
        return (
            ...
            '<span class="portfolio-category-badge" data-ar="' + escHtml(cat.ar) + '" data-en="' + escHtml(cat.en) + '" data-fr="' + escHtml(cat.fr) + '">' + escHtml(catText) + '</span>' +
            '<img src="' + escHtml(p.image) + '" alt="' + escHtml(titleText) + '" class="w-full h-72 object-cover" onerror="this.src=\'https://via.placeholder.com/400x300?text=No+Image\'">' +
            '<div class="portfolio-enhanced-overlay">' +
            '<div class="portfolio-icon-wrapper"><i class="fas ' + escHtml(icon) + '"></i></div>' +
            '<h3 class="text-xl font-bold text-white mb-2 heading-font" data-ar="' + escHtml(title.ar) + '" data-en="' + escHtml(title.en) + '" data-fr="' + escHtml(title.fr) + '">' + escHtml(titleText) + '</h3>' +
            '<p class="text-accent portfolio-desc" data-ar="' + escHtml(galleryShortDesc(p.description && p.description.ar)) + '" data-en="' + escHtml(galleryShortDesc(p.description && p.description.en)) + '" data-fr="' + escHtml(galleryShortDesc(p.description && p.description.fr)) + '">' + escHtml(shortDesc) + '</p>' +
            '</div></div>'
        );
    }).join('');
...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    if (galleryEl && typeof window.loadPortfolioItems === 'function') {
        ...
        if (items.length) {
            renderPortfolioGallery(items, galleryEl);
        }
    }
    ...
    document.addEventListener('languageChanged', function () {
        renderPortfolioGallery(window.__portfolioGalleryItems || [], document.getElementById('portfolioGallery') || document.createElement('div'));
